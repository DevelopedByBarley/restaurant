...
function Index() {
    ...
    function handleDeleteConfirm(e) {
        ...
        destroy("/admin/" + selectedAdminId, {
            ...
            onError: (error) => {
                console.error("Hiba történt az adminisztrátor törlésekor:", error);
            },
        });
    ...
    return (
        <div className="xl:ml-72">
            <section className="max-w-6xl w-full p-8 bg-white rounded-md shadow-md dark:bg-gray-800">
                <h2 className="text-2xl font-bold text-gray-800 dark:text-white mb-2">
                    Admin Vezérlőpult
                </h2>

                <p className="text-sm text-gray-600 dark:text-gray-300">
                    Használd a bal oldali menüt a navigációhoz, vagy válassz egy funkciót a kezdéshez.
                </p>
            </section>

            <section>
                <div className=" max-w-4xl w-full p-6 md:grid grid-cols-5 grid-rows-5 gap-4 min-h-[600px]">
                    <div className="col-span-3 row-span-3 border">1</div>
                    <div className="col-span-2 row-span-3 col-start-4 border">2</div>
                    <div className="col-span-2 row-span-2 row-start-4 border">3</div>
                    <div className="col-span-2 row-span-2 col-start-3 row-start-4 border">4</div>
                    <div className="row-span-2 col-start-5 row-start-4 border">5</div>
                </div>
            </section>
            ...
            <section>
                {admins && admins.data.length > 1 ? (
                    <>
                        <div className="max-w-4xl w-full p-6 bg-white rounded-md shadow-md dark:bg-gray-800 mt-6">
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr>
                                        <th className="border-b px-4 py-2 text-gray-700 dark:text-gray-300">Név</th>
                                        <th className="border-b px-4 py-2 text-gray-700 dark:text-gray-300">Email</th>
                                        <th className="border-b px-4 py-2 text-gray-700 dark:text-gray-300">Műveletek</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {admins.data
                                        .filter(admin => admin.id !== auth.admin.id)
                                        .map(admin => (
                                            <tr
                                                key={admin.id}
                                                className="bg-gray-100 dark:bg-gray-700 even:bg-gray-200 even:dark:bg-gray-600"
                                            >
                                                <td className="border-b px-4 py-2 text-gray-800 dark:text-white">{admin.name}</td>
                                                <td className="border-b px-4 py-2 text-gray-800 dark:text-white">{admin.email}</td>
                    ...
                    <div className="max-w-4xl w-full p-6 bg-white rounded-md shadow-md dark:bg-gray-800 mt-6">
                        <p className="text-gray-600 dark:text-gray-300">Nincsenek adminisztrátorok.</p>
                    </div>
                )}
            </section>
        </div>
